...
@section('title', 'Добавление справки')
    ...
    <div class="wrapper">
        <div class="container">
            <div class="content_space">
                <a href="{{ route('certificates') }}" class="back">
                ...
                <div class="form_container">
                    <form action="{{ route('update_certificate', $certificate->id) }}" method="POST" autocomplete="off">
                        @csrf
                        <h2>Редактирование "{{ $certificate->name }}"</h2>
                        <div class="load_file_container">
                            <img src="{{ asset('/images/scan.png') }}" alt="Загрузка скана">
                        </div>
                        <div class="inputs_container">
                            <div class="input_wrapper">
                                <label for="name">Справка</label>
                                <input type="text" value="{{ $certificate->name }}" name="name" id="name"/>
                            </div>
                            <div class="input_wrapper">
                                <label for="address">Адрес</label>
                                <input type="text" value="{{ $certificate->address }}" name="address" id="address"/>
                            </div>
                            <div class="input_wrapper">
                                <label for="doctor">Доктор</label>
                                <input type="text" value="{{ $certificate->doctor }}" name="doctor" id="doctor"/>
                            </div>
                            <div class="input_wrapper">
                                <label for="started_at">Дата</label>
                                <input type="date" value="{{ $certificate->started_at }}" name="started_at" id="started_at"/>
                            </div>
                            <div class="input_wrapper wide">
                                <label for="description">Описание</label>
                                <textarea rows="3" name="description" id="description">{{ $certificate->description }}</textarea>
                            </div>
                        </div>
                        ...
                        <div style="display: flex; justify-content: flex-end">
                            <button class="form_button creation_button">Сохранить</button>
                        </div>
                    </form>
                </div>
            </div>
